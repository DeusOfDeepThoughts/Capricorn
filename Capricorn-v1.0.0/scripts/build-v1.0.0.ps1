...
$CacheRoot = Join-Path $SystemTemp ("CapricornV1.0.0Build-" + $PID)
$Build = Join-Path $CacheRoot "qt"
$Stage = Join-Path $CacheRoot "stage"
$CoreBinary = Join-Path $CacheRoot 'CapricornCore-v1.0.0.exe'
$PublishedStage = Join-Path $Root "Capricorn-v1.0.0-Windows-x64"
$PublishedStageTemp = Join-Path $Root (".Capricorn-v1.0.0-Windows-x64-publishing-" + $PID)
$PublishedStageBackup = Join-Path $Root (".Capricorn-v1.0.0-Windows-x64-backup-" + $PID)
$ArchiveTemp = Join-Path $CacheRoot "Capricorn-v1.0.0-Windows-x64.zip"
$Archive = Join-Path $Root "Capricorn-v1.0.0-Windows-x64.zip"
$ArchivePublishTemp = Join-Path $Root (".Capricorn-v1.0.0-Windows-x64-publishing-" + $PID + ".zip")
$ArchiveBackup = Join-Path $Root (".Capricorn-v1.0.0-Windows-x64-backup-" + $PID + ".zip")
$InstallerScript = Join-Path $Root "installer/Capricorn.iss"
$InstallerOutput = Join-Path $CacheRoot "installer-output"
$SetupTemp = Join-Path $InstallerOutput "Capricorn-v1.0.0-Setup-x64.exe"
$Setup = Join-Path $Root "Capricorn-v1.0.0-Setup-x64.exe"
$SetupPublishTemp = Join-Path $Root (".Capricorn-v1.0.0-Setup-x64-publishing-" + $PID + ".exe")
$SetupBackup = Join-Path $Root (".Capricorn-v1.0.0-Setup-x64-backup-" + $PID + ".exe")

trap {
    if (Test-Path -LiteralPath $CacheRoot) {
        Remove-Item -LiteralPath $CacheRoot -Recurse -Force -ErrorAction SilentlyContinue
    }
    foreach ($path in @($PublishedStageTemp,
                        $ArchivePublishTemp,
                        $SetupPublishTemp)) {
...
$ExpectedProjectName = "Capricorn-v1.0.0"
...
if (-not (Test-Path -LiteralPath $ProjectNameFile -PathType Leaf) -or
    ...
    throw "Project metadata is incomplete. Expected Capricorn-v1.0.0 / 1.0.0."
}
...
if ($FrontendCMakeText -notmatch 'project\(Capricorn-v1\.0\.0 VERSION 1\.0\.0' -or
    $MainSourceText -notmatch 'setApplicationName\(QStringLiteral\("Capricorn-v1\.0\.0"\)\)' -or
    $MainSourceText -notmatch 'setApplicationVersion\(QStringLiteral\("1\.0\.0"\)\)') {
    throw "Project sources are not consistently versioned as Capricorn-v1.0.0 1.0.0."
}
...
function Find-QtRoot {
    ...
    $pathQMake = Find-CommandPath 'qmake.exe'
    if ($pathQMake) { $candidates += (Split-Path -Parent (Split-Path -Parent $pathQMake)) }
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path -LiteralPath (Join-Path $candidate 'bin/qmake.exe') -PathType Leaf)) {
            return (Resolve-Path -LiteralPath $candidate).Path
        }
    }
    if ($QtRoot) { throw "The specified Qt root is invalid: $QtRoot" }
...
if ($IsMingw) {
    if (-not $ToolchainBin) {
        ...
        if ($toolchains.Count -gt 0) {
            ...
            $pathCompiler = Find-CommandPath 'g++.exe'
            $pathMake = Find-CommandPath 'mingw32-make.exe'
            if ($pathCompiler -and $pathMake -and
                (Split-Path -Parent $pathCompiler) -eq (Split-Path -Parent $pathMake)) {
                $ToolchainBin = Split-Path -Parent $pathCompiler
            } else {
                throw "The Qt MinGW toolchain was not found under $QtTools or in PATH. Pass -ToolchainBin."
            }
        }
    }
...
if ($GoVersionLine -notmatch 'go(\d+\.\d+(?:\.\d+)?)') { throw "Unable to detect the Go version." }
$GoVersionText = $matches[1]
if ([Version]$GoVersionText -lt [Version]'1.23.0') { throw "Go 1.23+ is required; found $GoVersionText" }
if (-not (Test-Path -LiteralPath $InstallerScript -PathType Leaf)) { throw "Installer script is missing: $InstallerScript" }
$ISCC = Find-InnoSetupCompiler

Write-Host "Building Capricorn-v1.0.0"
Write-Host "Qt:      $QtVersionText ($QtSpec)"
Write-Host "CMake:   $CMakeVersionText"
Write-Host "Go:      $GoVersionLine"
...
try {
    ...
    & $Go build -trimpath -ldflags '-s -w -H=windowsgui' -o $CoreBinary .
...
if (Test-Path -LiteralPath $PublishedStageTemp) { Remove-Item -LiteralPath (Assert-WorkspacePath $PublishedStageTemp) -Recurse -Force }

$Generator = if ($IsMingw) { 'MinGW Makefiles' } else { 'Visual Studio 17 2022' }
$configure = @('-S', $Root, '-B', $Build, '-G', $Generator, "-DCMAKE_BUILD_TYPE=$Configuration", '-DCMAKE_INSTALL_BINDIR=.', "-DCAPRICORN_CORE_BINARY=$CoreBinary")
...
$required = @('Capricorn-v1.0.0.exe','Qt6Core.dll','Qt6Gui.dll','Qt6Widgets.dll','Qt6Network.dll',
    'Qt6Svg.dll','Qt6Sql.dll','qt.conf','plugins/platforms/qwindows.dll',
    'plugins/sqldrivers/qsqlite.dll','core/CapricornCore-v1.0.0.exe')
...
$SmokeMarker = Join-Path $CacheRoot 'startup-smoke.ok'
$SmokeProfile = Join-Path $CacheRoot 'smoke-profile'
$SmokeRoamingAppData = Join-Path $SmokeProfile 'AppData/Roaming'
$SmokeLocalAppData = Join-Path $SmokeProfile 'AppData/Local'
New-Item -ItemType Directory -Path $SmokeRoamingAppData, $SmokeLocalAppData -Force | Out-Null
...
try {
    ...
    $smokeProcess = Start-Process -FilePath (Join-Path $Stage 'Capricorn-v1.0.0.exe') -ArgumentList @('--startup-smoke', $SmokeMarker) -WorkingDirectory $Stage -PassThru
    if (-not $smokeProcess.WaitForExit(15000)) {
        $smokeProcess.Kill()
        $smokeProcess.WaitForExit()
        throw 'Release validation failed; startup smoke test timed out.'
    }
    if ($smokeProcess.ExitCode -ne 0 -or -not (Test-Path -LiteralPath $SmokeMarker -PathType Leaf)) {
        throw "Release validation failed; startup smoke test exited with code $($smokeProcess.ExitCode)."
    }
} finally {
...
# Prepare every publishable artifact before touching the previous release.
[IO.File]::Copy($ArchiveTemp, $ArchivePublishTemp, $true)
[IO.File]::Copy($SetupTemp, $SetupPublishTemp, $true)
$archiveHash = (Get-FileHash -LiteralPath $ArchivePublishTemp -Algorithm SHA256).Hash.ToLowerInvariant()
$setupHash = (Get-FileHash -LiteralPath $SetupPublishTemp -Algorithm SHA256).Hash.ToLowerInvariant()
...
$publishExpandedStage = $Root.Length -le 150
if ($publishExpandedStage) {
    New-Item -ItemType Directory -Path $PublishedStageTemp -Force | Out-Null
    [IO.Compression.ZipFile]::ExtractToDirectory($ArchiveTemp, $PublishedStageTemp)
} else {
...
# Commit all artifacts as one recoverable publish operation. If any move fails,
# restore every previous artifact before reporting the build failure.
...
try {
    if (Test-Path -LiteralPath $Archive) {
        [IO.File]::Move($Archive, $ArchiveBackup)
        $archiveBackedUp = $true
    }
    if (Test-Path -LiteralPath $Setup) {
        [IO.File]::Move($Setup, $SetupBackup)
        $setupBackedUp = $true
    }
    if ($publishExpandedStage -and (Test-Path -LiteralPath $PublishedStage)) {
        [IO.Directory]::Move($PublishedStage, $PublishedStageBackup)
        $stageBackedUp = $true
    }

    [IO.File]::Move($ArchivePublishTemp, $Archive)
    $archivePublished = $true
    [IO.File]::Move($SetupPublishTemp, $Setup)
    $setupPublished = $true
    if ($publishExpandedStage) {
        [IO.Directory]::Move($PublishedStageTemp, $PublishedStage)
        $stagePublished = $true
    }
} catch {
    $publishError = $_
    ...
    foreach ($published in @(
        @{ Enabled = $stagePublished; Path = $PublishedStage; Directory = $true },
        @{ Enabled = $setupPublished; Path = $Setup; Directory = $false },
        @{ Enabled = $archivePublished; Path = $Archive; Directory = $false }
    )) {
        if ($published.Enabled -and (Test-Path -LiteralPath $published.Path)) {
            try {
                Remove-Item -LiteralPath $published.Path -Recurse:$published.Directory -Force
            } catch {
                $recoveryErrors += "Could not remove incomplete artifact '$($published.Path)': $($_.Exception.Message)"
            }
        }
    }

    foreach ($backup in @(
        @{ Enabled = $stageBackedUp; Backup = $PublishedStageBackup; Target = $PublishedStage; Directory = $true },
        @{ Enabled = $setupBackedUp; Backup = $SetupBackup; Target = $Setup; Directory = $false },
        @{ Enabled = $archiveBackedUp; Backup = $ArchiveBackup; Target = $Archive; Directory = $false }
    )) {
        if ($backup.Enabled -and (Test-Path -LiteralPath $backup.Backup)) {
            try {
                if ($backup.Directory) {
                    [IO.Directory]::Move($backup.Backup, $backup.Target)
                } else {
                    [IO.File]::Move($backup.Backup, $backup.Target)
                }
            } catch {
                $recoveryErrors += "Could not restore backup '$($backup.Backup)' to '$($backup.Target)': $($_.Exception.Message)"
            }
        }
    }

    foreach ($recoveryError in $recoveryErrors) {
        Write-Warning $recoveryError
    }
    throw $publishError
}

foreach ($path in @($PublishedStageBackup, $ArchiveBackup, $SetupBackup)) {
    if (Test-Path -LiteralPath $path) {
        Remove-Item -LiteralPath $path -Recurse -Force
    }
}

$publishedExecutable = if ($publishExpandedStage) {
    Join-Path $PublishedStage 'Capricorn-v1.0.0.exe'
} else { $null }

Write-Host ""
if ($publishedExecutable) { Write-Host "v1.0.0 executable: $publishedExecutable" }
Write-Host "v1.0.0 runtime ZIP: $Archive"
Write-Host "v1.0.0 installer:   $Setup"
