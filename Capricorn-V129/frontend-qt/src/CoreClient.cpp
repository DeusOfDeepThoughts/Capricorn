...
void CoreClient::request(const QByteArray &method, const QString &path, const QJsonObject &body,
...
void CoreClient::createPersonaSession(const QJsonObject &config, const QString &profile, int strength,
                                      ...
                                      Callback callback) {
    // Provider handoff uses three bounded layers: stable pet profile, compressed
    // user/relationship memory, and a small recent-turn window. The full local
    // transcript is never placed into the model context.
    request(QByteArrayLiteral("POST"), QStringLiteral("/v1/model/persona-session"), QJsonObject{
        ...
        {QStringLiteral("longMemoryMarkdown"), longMemoryMarkdown},
        {QStringLiteral("recentMessages"), recentMessages}
    }, std::move(callback), 360000);
}
void CoreClient::chat(const QString &sessionId, const QString &message, Callback callback) {
    ...
    request(QByteArrayLiteral("POST"), QStringLiteral("/v1/model/chat"),
            QJsonObject{{QStringLiteral("sessionId"), sessionId}, {QStringLiteral("prompt"), message}},
            std::move(callback), 360000);
}
void CoreClient::rebuildMemory(const QString &sessionId, const QJsonArray &messages, Callback callback) {
    request(QByteArrayLiteral("POST"), QStringLiteral("/v1/model/memory/rebuild"),
