...
QString AppData::stateFilePath() const {
    const QString root = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation);
    QDir().mkpath(root);
    return root + QStringLiteral("/state-v129.json");
}
...
bool AppData::load(QString *error) {
    ...
    // Previous-version → V129 state migration, with earlier fallbacks. Skip damaged candidates rather
    // than stopping at the first non-empty file and silently resetting user data.
    if (!stateLoaded) {
        const QString root = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation);
