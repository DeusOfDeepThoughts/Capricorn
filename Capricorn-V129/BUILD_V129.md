...
```powershell
.\scripts\build-v129.ps1 -QtRoot "C:\Qt\6.11.1\mingw_64"
```

脚本只编译一次 Qt 与 Go，然后从同一个经过校验的 staging 目录同时生成：

- `Capricorn-V129-Windows-x64.zip` —— 便携版，用户解压后直接运行
- `Capricorn-V129-Setup-x64.exe` —— 安装版，用户双击后选择安装目录
- `Capricorn-V129-Windows-x64\` —— 当源码路径不太深时额外保留的展开运行目录
...
```powershell
.\scripts\build-v129.ps1 -NoAutoInstallInno
```

也可以直接指定：

```powershell
.\scripts\build-v129.ps1 -InnoSetupCompiler "C:\Program Files\Inno Setup 7\ISCC.exe"
```

## 安装版行为

`Capricorn-V129-Setup-x64.exe`：
...
- 使用固定 AppId，为以后 V130、V131 等覆盖升级保留稳定的安装身份
...
V129 的构建流程已经为 Setup.exe 留好标准安装器结构，但源码包**不包含任何开发者证书或私钥**。如果正式公开发布，后续可在构建链中另外接入 Authenticode 代码签名；没有签名时 Windows SmartScreen 可能显示“未知发布者”，这与安装器能否正常安装无关。
