...
V129 基于 V128，仅增强发布/安装构建链，不改变 Capricorn 的 UI、业务逻辑、人格、聊天、语音或桌宠行为。

## 新增：一次编译同时生成两种发行物

执行 `BUILD_V129.cmd` 后，同一份经过校验的 staging 目录会同时生成：

- `Capricorn-V129-Windows-x64.zip`
...
新增 `installer/Capricorn.iss`，采用 Inno Setup：
